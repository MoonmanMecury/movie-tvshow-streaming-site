...
import { supabase } from '../supabase'; // 1. Ensure this path is correct
...
const WatchPage = () => {
  ...
  const [isSaving, setIsSaving] = useState(false); // Loading state for button
  ...
  const handleVault = async () => {
    ...
    try {
      ...
      if (!user) {
        alert("Authentication required for Vault access.");
        return navigate('/login');
      }

      const { error } = await supabase
        .from('watchlist') // Ensure your table is named 'watchlist'
        .upsert({
          user_id: user.id,
          content_id: id,
          type: type,
      ...
      alert("Signal Secured to Vault.");
      
    } catch (error) {
      console.error("Vault Error:", error.message);
      alert("Failed to secure signal.");
    } finally {
      setIsSaving(false);
    }
  };
          ...
          <div className="xl:col-span-3 space-y-8">
            ...
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="space-y-6 px-2 md:px-0">
              ...
              <div className="flex flex-wrap gap-4 pt-4">
                {/* 3. ATTACHED HANDLEVAULT */}
                <button 
                  onClick={handleVault}
                  disabled={isSaving}
                  className={`flex-1 md:flex-none px-10 py-4 bg-white text-black text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-red-600 hover:text-white transition-all shadow-xl active:scale-95 ${isSaving ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  {isSaving ? 'Securing...' : 'Secure to Vault'}
                </button>
                <button 
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      <select 
                        ...
                        className="w-full bg-black/60 border border-white/10 p-4 rounded-2xl text-[11px] font-black uppercase tracking-widest outline-none appearance-none cursor-pointer"
                      >
                        {[...Array(details?.number_of_seasons)].map((_, i) => (
                          <option key={i+1} value={i+1} className="bg-[#050000]">Level {i+1}</option>
                        ))}
                      </select>
